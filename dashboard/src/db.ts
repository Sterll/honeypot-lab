...
import type { CowrieEvent, DashboardStats, GeoInfo } from "./types";
...
export function insertEvent(event: CowrieEvent): void {
  try {
    const stmt = db.prepare(`
      INSERT INTO events (eventid, timestamp, session, src_ip, src_port, dst_port, message, username, password, command, duration, url, shasum, ssh_version, hassh, raw)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
...
export function getEvents(limit = 100, offset = 0): CowrieEvent[] {
...
export function getStats(): DashboardStats {
  const row = db.prepare(`
    SELECT
      (SELECT COUNT(*) FROM events WHERE eventid = 'cowrie.session.connect') as totalConnections,
      (SELECT COUNT(DISTINCT src_ip) FROM events) as uniqueIps,
      (SELECT COUNT(*) FROM events WHERE eventid IN ('cowrie.login.failed', 'cowrie.login.success')) as credentialsCaptured,
      (SELECT COUNT(DISTINCT session) FROM events WHERE eventid = 'cowrie.session.connect') as sessionsCount,
      (SELECT COUNT(*) FROM events WHERE eventid = 'cowrie.command.input') as commandsExecuted,
...
export function getSessions(): { session: string; src_ip: string; startTime: string; commandCount: number; loginSuccess: number }[] {
  return db.prepare(`
    SELECT
      e.session,
      e.src_ip,
      MIN(e.timestamp) as startTime,
      SUM(CASE WHEN e.eventid = 'cowrie.command.input' THEN 1 ELSE 0 END) as commandCount,
      SUM(CASE WHEN e.eventid = 'cowrie.login.success' THEN 1 ELSE 0 END) as loginSuccess
    FROM events e
    WHERE e.session IS NOT NULL
    GROUP BY e.session
    ORDER BY startTime DESC
    LIMIT 100
  `).all() as { session: string; src_ip: string; startTime: string; commandCount: number; loginSuccess: number }[];
}
